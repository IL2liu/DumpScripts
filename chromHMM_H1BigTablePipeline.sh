...
if [ $# -eq 0 ]; then
  echo "Usage:chromHMM_H1BigTablePipeline.sh -r [n] -i [input files] -c[segment file] -o [output]"
  echo "e.g ./chromHMM_H1BigTablePipeline.sh -r 0.2 -i 'TEST/*' -c segment.txt -o TEST/bigTable.bed"
  echo "-r [n] 0.2 for 3000bp and 0.02 for 300bp"
  echo "segment file is a bed format with one extra column for segmented chromatin states by chromHMM"
exit 1
fi

while getopts ":r:i:c:o:" opt; do
  case $opt in
    r)
    ...
    i)
      ...
      ;;
    c)
      echo "-c your segment file is: $OPTARG" >&2
      segment="$OPTARG"
      ;;
    o)
...
#start merging
./mergeDiscretizedBed.py $input_files > $dir/$merged_file 

#combine the merged files with the segmentation file
mapped_file="mapped-"$merged_file

#add header to segmentation file
segment_file=${segment##*/};
segment_file="header-"$segment_file
awk 'BEGIN{print "chr\tstart\tend\tstate"}1' $segment > $dir/$segment_file
paste $dir/$segment_file $dir/$merged_file > $dir/$mapped_file

#remove the lines where all the reads of all the columns for each line are zeros
Nonzero_file="NonZero-"$mapped_file
./removeZeroAnnotation.py $dir/$mapped_file > $dir/$Nonzero_file
