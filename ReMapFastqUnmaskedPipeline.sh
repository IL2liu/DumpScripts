...
# gem version: 1.366                                               # 
# use the unmasked gem-index, allowing for low complexity regions  #
# this indexed built from unmasked-hg19                            #
####################################################################
...
do
  ...
  gem-mapper -I 'hg19_unmasked_Gemindex/hg19_unmasked.gem' -i $i -q 'offset-33' -o $output_dir/$i_file -T $core && echo "Gem mapping of $i:OK" >&2;
  map_f=$i_file.map
  ...
  if [[ $file_size -ne 0 ]]; then
    ...
    sam_f=$map_f.sam
    gem-2-sam -I 'hg19_unmasked_Gemindex/hg19_unmasked' -i $output_dir/$map_f -q 'offset-33' -o $output_dir/$sam_f && echo "Sam conversion of $map_f:OK" >&2;
  else
