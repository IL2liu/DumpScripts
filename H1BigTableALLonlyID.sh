...
if [ $# -eq 0 ]; then
  echo "Usage:H1BigTableALLonlyID.sh -i [input files] -o [output]"
  echo "e.g ./H1BigTableALLonlyID.sh -i 'TEST/*' -o TEST/bigTable.bed"
exit 1
fi
...
#start merging
./mergeOnlyIDBed.py $input_files > $dir/$merged_file 
