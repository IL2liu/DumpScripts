...
if [ $# -eq 0 ]; then
  echo "Usage -i [input_file] -n [no.core] -m [mismatches] -o [output_dir]"
...
do
  ...
  i_file=${i##*/}; 
  filename="${i_file%.*.*}"
  gunzip -c $i | gem-mapper -I '/software/private/gfilion/seq/gem/hg19/hg19_masked.gem' -q 'ignore' --mismatch-alphabet "ACGTN" -m $mismatch --unique-mapping -o $output_dir/$filename -T $core && echo "Gem mapping of $i:OK" >&1;
done                                            
