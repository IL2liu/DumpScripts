...
setwd("/TEMP_DDN/users/gfilion/rlim/VirusAlignment/Ranalysis/HeLa_input/")
...
infectionScores_001 <- read.delim("sorted_Normalized_virus_infected_001.txt", header=F)

# obtain the number of viral infections
virus_infected_001 <- read.delim("virus_infected_001.txt", header=F)
...
gp + guides(fill=FALSE)+coord_cartesian(ylim=c(0.0, max(plotSample_001$Scores)+0.01)) +
  annotate("text", label = annot_threshold, x = 27, y = max_threshold+0.003, size = 5, colour = "red")
