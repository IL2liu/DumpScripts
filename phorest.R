...
#' By default, phorest() expects the following columns: phenotype,phenotypeGroup,SNP,SNPGroup,value,lowerBound and upperBound.
#' But this can be over-ridden with the columnNames, phenotypeGroups and SNPGroups arguments.
#' SNP are assumed to be of the form "rsXXXXX"; this is used for ordering labels in the legend.
#'
...
#'   value=rnorm(nSNP*nPhenotype,mean=0,sd=1),
...
phorest<-function(
  ...
  columnNames=NULL,
  phenotypeGroups=NULL,
  unassignedGroup='unassigned', 
  transparency=0.6,
  legendTransparency=FALSE,
  connectingLines=FALSE, 
  connectingLinesColour="#707070",
  legendSNP=TRUE,
  xLabel=NULL,
  yLabel=NULL,
  title='',
...
## sanity checks
  if(!require("scales")) stop("Requires scales")
  if(!require("ggplot2")) stop("Requires ggplot2")
  if(!require("plyr")) stop("Requires plyr")

# ## Is ggplot2 version => 0.9?
  earlyVersion<-compareVersion(packageDescription("ggplot2",fields="Version"),"0.9.0")<0

  if(earlyVersion) stop("Too old version of ggplot2. Please update to at least ggplot 0.9.") 
  
# Necessary columns
  if(!is.null(columnNames)){
    namesColumn<-names(columnNames)
    names(namesColumn)<-columnNames
    associations<-plyr::rename(associations,namesColumn)
  }
  if(largeSNPSet)
...
## build a 'group' column if necessary
  # apply the group definition to a column
  ...
  # Check whether association$group exists
  if(!"phenotypeGroup" %in% names(associations)){ # no group columns
  ...
  if(!is.factor(associations$phenotypeGroup)) 
   ...
   if(largeSNPSet| (("SNPGroup" %in% names(associations) | !is.null(SNPGroups))))
    if(!"SNPGroup" %in% names(associations)){ # no SNPGroup column
      associations$SNPGroup<-applyLookup(SNPGroups,associations$SNP,unassignedGroup)      
    }
  
  # sort by groups/phenotypes
  idx<-order(
    sapply(as.character(unique(associations$SNP)),
           function(s) as.numeric(tail(strsplit(s,'s')[[1]],1)),
           USE.NAMES=FALSE))
  ...
  if(length(transparency) %in% c(1,nrow(associations)))
     associations$transparency<-transparency
  else if(warningOn) warning("transparency has been recycled.")
...
#       summaryAssociations<-ddply(
#         associations,
#         .(SNPGroup,phenotype,phenotypeGroup),
#         summarise,
#         aggregatedValue=aggregatingFunction(value)) # known bug in Plyr (http://stackoverflow.com/questions/4289602/plyr-summarize-only-calls-global-functions)
      summaryAssociations<-aggregate(value~SNPGroup+phenotype+phenotypeGroup,data=associations,FUN=aggregatingFunction)

      summaryLowerBound<-aggregate(value~SNPGroup+phenotype+phenotypeGroup,data=associations,FUN=aggregatingLowerBound)
      summaryLowerBound<-plyr::rename(summaryLowerBound,c("value"="lowerBound"))

      summaryUpperBound<-aggregate(value~SNPGroup+phenotype+phenotypeGroup,data=associations,FUN=aggregatingUpperBound)
      summaryUpperBound<-plyr::rename(summaryUpperBound,c("value"="upperBound"))
      ...
      summaryAssociations<-arrange(summaryAssociations,phenotypeGroup,desc(phenotype))
      summaryAssociations$phenotype<-factor(summaryAssociations$phenotype,unique(summaryAssociations$phenotype))
      idx<-order(as.character(unique(summaryAssociations$SNPGroup)))
      summaryAssociations$SNP<-factor(summaryAssociations$SNPGroup,unique(summaryAssociations$SNPGroup)[idx])
      p<-p+geom_path(
        aes(x=value,y=phenotype,group=SNPGroup,colour=SNPGroup),
        alpha=transparencyAggregate,
        size=1.2,
      ...
      for(g in unique(summaryAssociations$SNPGroup)){
        ...
        datapoly<-rbind(datapoly,data.frame(
          SNPGroup=g,
          value=c(p1$lowerBound,rev(p1$upperBound)),
          phenotype=c(as.character(p1$phenotype),rev(as.character(p1$phenotype))),
          vertexOrder=1:(2*nrow(p1)))              
          )
      }
      ...
      if(!legendSNP) p<-p+scale_color_hue(guide='none')+scale_fill_hue(guide='none')
      
#     }
#     else p<-p+geom_path(aes(group=SNP),colour='white',alpha=transparency) 

    
    p<-p+opts(
      panel.background=theme_rect(fill='black'),
    ...
    # Legends
    if(!legendSNP) p<-p+scale_color_hue(guide='none') 
    if(!legendTransparency) p<-p+scale_alpha_identity(guide='none')
    ...
    p<-p+facet_grid(phenotypeGroup~.,scales="free",space="free")+opts(strip.text.y = theme_text()) 
  
  # Labels
  if(!is.null(xLabel))
    if(is.na(xLabel)) p<-p+opts( axis.title.x=theme_blank()) else p<-p+xlab(xLabel)
  if(!is.null(yLabel))
    if(is.na(yLabel)) p<-p+opts( axis.title.y=theme_blank()) else p<-p+ylab(yLabel)
  
  p<-p+opts(title=title)    
  p
}
# transparencyScale: scale_alpha_identity(legend=FALSE)
# linetype: scale_linetype(legend=FALSE)
# 
