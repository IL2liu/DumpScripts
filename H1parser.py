      ...
      if item[:2] == '#T':
         ...
         with open(f_os, 'w') as f:
            ...
            while True:
               ...
               status = status + chr(8)*(len(status)+1) + '\t' + file_name
               sys.stderr.write(status + "\n")

if __name__ == '__main__':

   #getting human H1-hESC
   url = "http://genome.ucsc.edu/cgi-bin/hgFileSearch?hgsid=296064723&db=hg19&hgt_tsDelRow=&hgt_tsAddRow=&tsName=&tsDescr=&tsGroup=Any&fsFileType=Any&hgt_mdbVar1=cell&hgt_mdbVal1=H1-hESC&hgt_mdbVar2=antibody&hgt_mdbVal2=Any&hgfs_Search=search"
   
   #parse the searchtable
   tableRecords = parseSearchTable(url)
   
   #write the annotation file for H1-hESC
   writeAnnot(tableRecords, 'H1-hESC')

   #get the files
   #run the script with 2> log.txt
   Download_file = RetrieveLink("H1-hESC_annotation.txt")
