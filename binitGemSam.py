...
class binThread (multiprocessing.Process):
   ...
   Binning in threads
   """
   def __init__(self, lock, queue, bin_size, output_dir, format):
      self.lock = lock
      self.queue = queue
   ...
   def run(self):
      while True:
         ...
         binit(bin_size=self.bin_size, format=self.format,  output_dir=self.output_dir, input_file=input_f)
...
if __name__ == "__main__":
   ...
   threads = [binThread(lock, queue, args.bin_size, args.output_dir, args.format) for i in range(args.n_threads)]
